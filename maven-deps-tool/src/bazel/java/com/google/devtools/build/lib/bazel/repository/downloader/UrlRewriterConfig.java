...
/**
 * Models the downloader config file. This file has a line-based format, with each line starting with a directive and
 * then the action to take. These directives are:
 *
 * <ul>
 ...
 * <li>{@code rewrite pattern pattern} Rewrite a URL using the given pattern. Back references are numbered from `$1`
 * <li>{@code all_blocked_message message which may contain spaces} If the rewriter causes all URLs for a particular
 * resource to be blocked, this informational message will be rendered to the user. This directive may only be present
 * at most once.
 * </ul>
 *
 ...
 *
 * <p>
 * Comments within the config file are allowed, and must be on their own line preceded by a {@code #}.
 */
public class UrlRewriterConfig {
     ...
     *             If any processing problems occur.
     */
    public UrlRewriterConfig(String filePathForErrorReporting, Reader config) throws UrlRewriterParseException {
        final ImmutableSet.Builder<String> allowList = ImmutableSet.builder();
        final ImmutableSet.Builder<String> blockList = ImmutableSet.builder();
        final ImmutableMultimap.Builder<Pattern, String> rewrites = ImmutableMultimap.builder();
        String allBlockedMessage = null;

        try (var reader = new BufferedReader(config)) {
            var lineNumber = 1;
            for (var line = reader.readLine(); line != null; line = reader.readLine(), lineNumber++) {
                // Find the first word
                final var parts = SPLITTER.splitToList(line);
                // Allow comments to use #
                if (parts.isEmpty() || parts.get(0).startsWith("#")) {
                    continue;
                }

                final var location = Location.fromFileLineColumn(filePathForErrorReporting, lineNumber, 0);
    ...
    /**
     * Returns a {@link Map} of {@link Pattern} to match against, and the rewriting changes to apply when matched.
     */
